...
evEventLoop *evCreateEventLoop(void)
{
    ...
    if (evApiCreate(eventLoop) == -1) {
        free(eventLoop);
        return NULL;
    }
    /* Events with mask == AE_NONE are not set. So let's initialize the
     * vector with it. */
    for (i = 0; i < EVENT_SETSIZE; i++)
...
{
    if (fd >= EVENT_SETSIZE) return EVENT_ERR;
    evFileEvent *fe = &eventLoop->events[fd];

    if (evApiAddEvent(eventLoop, fd, mask) == -1)
        return EVENT_ERR;

    fe->mask |= mask;
    if (mask & EVENT_READABLE) fe->rfileProc = proc;
    if (mask & EVENT_WRITABLE) fe->wfileProc = proc;
...
{
    if (fd >= EVENT_SETSIZE) return;
    evFileEvent *fe = &eventLoop->events[fd];

    if (fe->mask == EVENT_NONE) return;
    fe->mask = fe->mask & (~mask);
    if (fd == eventLoop->maxfd && fe->mask == EVENT_NONE) {
        /* Update the max fd */
        int j;
...
int evDeleteTimeEvent(evEventLoop *eventLoop, long long id)
{
    ...
    return EVENT_ERR; /* NO event with the specified ID found */
}
...
{
    evTimeEvent *te = eventLoop->timeEventHead;
...
int evProcessEvents(evEventLoop *eventLoop, int flags)
{
    ...
    if (eventLoop->maxfd != -1 ||
        ...
        for (j = 0; j < numevents; j++) {
            evFileEvent *fe = &eventLoop->events[eventLoop->fired[j].fd];
...
int evWait(int fd, int mask, long long milliseconds) {
    ...
    if ((retval = select(fd+1, &rfds, &wfds, &efds, &tv)) > 0) {
        if (FD_ISSET(fd,&rfds)) retmask |= EVENT_READABLE;
        if (FD_ISSET(fd,&wfds)) retmask |= EVENT_WRITABLE;
        return retmask;
    } else {
        return retval;
    }
...
void evMain(evEventLoop *eventLoop) {
    ...
    while (!eventLoop->stop) {
        ...
        evProcessEvents(eventLoop, EVENT_ALL_EVENTS);
    }
}

...
struct __rollingfile_conf {
  ...
  log_rollingpolicy_t* rfc_policy;
};
...
****************************************************************************/
static int rollingfile_open(log_appender_t* this)
{
  int rc = 0; 
  rollingfile_udata_t *rfup = log_appender_get_udata(this);
 ...
 */
 if ( rfup->rfu_conf.rfc_policy ) {
   sd_debug("rollingfile udata has a policy '%s'--calling rollover",
          log_rollingpolicy_get_name( rfup->rfu_conf.rfc_policy)  );
   
   /*
   * If the policy is not yet initialized force it now to default values
   */
   if (!log_rollingpolicy_is_initialized(rfup->rfu_conf.rfc_policy)){
     sd_debug("policy not initialized, calling init now");
           log_rollingpolicy_init(rfup->rfu_conf.rfc_policy,
			   rfup);
   }
   
   if ( log_rollingpolicy_rollover(rfup->rfu_conf.rfc_policy,
			       &rfup->rfu_current_fp)){
                                rc = 1; /* rollover error */
   } else {
    rfup->rfu_current_file_size = 0;
...
static int rollingfile_append(log_appender_t* this,
             const log_logging_event_t* a_event)
{
  rollingfile_udata_t* rfup = log_appender_get_udata(this);
  log_logging_event_t* my_event =
  ...
  if ( rfup->rfu_conf.rfc_policy != NULL) {
    ...
    if( log_rollingpolicy_is_triggering_event(rfup->rfu_conf.rfc_policy,
					 a_event,		    
					 rfup->rfu_current_file_size)){
#ifdef __SD_DEBUG__
      ...
      if ( (rc = log_rollingpolicy_rollover(rfup->rfu_conf.rfc_policy,
...
/****************************************************************************/
static int rollingfile_close(log_appender_t* this)
{  
  int rc = 0;
  ...
  if(!this){
    ...
    rfup = log_appender_get_udata(this);
    ...
    if ( rfup->rfu_conf.rfc_policy){
      if (!log_rollingpolicy_fini(rfup->rfu_conf.rfc_policy)){
...
rollingfile_udata_t *rollingfile_make_udata(void){
  rollingfile_udata_t *rfup = NULL;
  rfup = (rollingfile_udata_t *)calloc(1, sizeof(rollingfile_udata_t));
...
int rollingfile_udata_set_logdir(rollingfile_udata_t* rfup, const char *logdir){
...
const char * rollingfile_udata_get_logdir(rollingfile_udata_t* rfup){
...
int rollingfile_udata_set_files_prefix( rollingfile_udata_t* rfup, const char* fprefix){
...
const char*  rollingfile_udata_get_files_prefix( rollingfile_udata_t* rfup){
...
long  rollingfile_get_current_file_size( rollingfile_udata_t* rfup){
...
/****************************************************************************/
const log_appender_type_t log_appender_type_rollingfile = {
    "rollingfile",
    rollingfile_open,
    rollingfile_append,
    rollingfile_close

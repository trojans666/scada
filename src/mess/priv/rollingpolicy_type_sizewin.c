...
static int sizewin_init(log_rollingpolicy_t *this, rollingfile_udata_t* rfup);
static int sizewin_rollover(log_rollingpolicy_t *this, FILE **current_fpp );
...
static int sizewin_is_triggering_event(
  ...
  long len = 0;
  int decision = 0;
  rollingpolicy_sizewin_udata_t *swup = log_rollingpolicy_get_udata(this);
  /*rollingfile_udata_t *rfup = log_rollingpolicy_get_udata(this);  */
  ...
  */
  ...
  if ( !swup || !swup->sw_logdir){
    sd_error("rollingpolicy '%s' not yet configured (logdir,prefix etc.)",
      log_rollingpolicy_get_name(this));
  } else {
...
static int sizewin_init(log_rollingpolicy_t *this, rollingfile_udata_t *rfup){
  ...
  swup = log_rollingpolicy_get_udata(this);
  if ( swup == NULL ){
    swup = sizewin_make_udata();
    log_rollingpolicy_set_udata(this, swup);
  }
...
static int sizewin_fini(log_rollingpolicy_t *this){
  rollingpolicy_sizewin_udata_t *swup =  NULL;
  int i = 0;
  int rc = 0;
  ...
  */
  ...
  free(swup);
  log_rollingpolicy_set_udata(this,NULL);
...
rollingpolicy_sizewin_udata_t *sizewin_make_udata(void){
  rollingpolicy_sizewin_udata_t *swup = NULL;
  swup = (rollingpolicy_sizewin_udata_t *)calloc(1,
...
int sizewin_udata_set_rfudata(rollingpolicy_sizewin_udata_t *swup,
...
static char **sizewin_make_filename_array(rollingpolicy_sizewin_udata_t *swup){
  ...
  filenames = (char **)calloc(swup->sw_conf.swc_file_max_num_files,
				 sizeof(char *));
